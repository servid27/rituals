...
const save = (k: string, v: any) => { if (hasLS()) { try { localStorage.setItem(k, JSON.stringify(v)); } catch {} } };
const load = <T,>(k: string, fb: T): T => { if (!hasLS()) return fb; try { const r = localStorage.getItem(k); return r ? (JSON.parse(r) as T) : fb; } catch { return fb; } };

// Versioning + touched flag (better UX; no big reset button)
const STORAGE_VERSION = "rk_v3"; // bump when seed/schema changes
const setVersion = () => { if (hasLS()) try { localStorage.setItem("rk_version", STORAGE_VERSION); } catch {} };
const getVersion = (): string | null => { if (!hasLS()) return null; try { return localStorage.getItem("rk_version"); } catch { return null; } };
const setTouched = () => { if (hasLS()) try { localStorage.setItem("rk_touched", "1"); } catch {} };
const getTouched = (): boolean => { if (!hasLS()) return false; try { return localStorage.getItem("rk_touched") === "1"; } catch { return false; } };
...
const useStore = () => {
  ...
  const addSession = (s: SessionRecord) => { setSessions((arr) => [s, ...arr]); setTouched(); };
...
// ---------- Small UI bits ----------
const NumberInput: React.FC<{ value: number; onChange: (n: number) => void; className?: string }> = ({ value, onChange, className }) => (
  <input
    type="number"
...
const DragHandle: React.FC = () => (
  <span className="cursor-grab text-gray-400 select-none" title="Drag">⋮⋮</span>
);
...
const RoutineView: React.FC<{
  ...
  const completeCurrent = () => {
    ...
    setDoneActuals((arr) => { const next = [...arr]; next[currentIndex] = actual; return next; });
    const nextIndex = currentIndex + 1;
    if (nextIndex < routine.tasks.length) { setCurrentIndex(nextIndex); setPerTaskElapsed(0); }
    else { setRunning(false); setCurrentIndex(null); }
  };

  const skipCurrent = () => {
    if (currentIndex == null) return;
    setDoneActuals((arr) => { const next = [...arr]; next[currentIndex] = 0; return next; });
    const nextIndex = currentIndex + 1;
    if (nextIndex < routine.tasks.length) { setCurrentIndex(nextIndex); setPerTaskElapsed(0); }
    else { setRunning(false); setCurrentIndex(null); }
  };
  ...
  const totalActual = doneActuals.reduce((a, v) => a + (typeof v === 'number' ? v : 0), 0) + (currentIndex != null ? perTaskElapsed : 0);
  ...
  const finish = () => {
    ...
    const rec: SessionRecord = {
      id: uid(), routineId: routine.id, dateISO: now.toISOString(), startISO,
      endISO: now.toISOString(), targetSeconds: totalTarget, actualSeconds: totalActual,
      deltaSeconds: totalActual - totalTarget, tasksCompleted: tasksDone, tasksTotal: routine.tasks.length,
    };
    onRecord(rec);
    reset();
    setFinishStats(rec);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className={`px-3 py-2 rounded-xl ${running ? 'bg-gray-200 text-black' : 'bg-green-600 text-white'}`} onClick={handleStartPause}>{running ? "⏸️ Pause" : "▶️ Start"}</button>
      ...
      <div className="rounded-2xl border bg-white">
        ...
        <ul className="divide-y">
          {routine.tasks.map((t, i) => {
            ...
            const remainingNow = isActive ? (t.targetSeconds - perTaskElapsed) : (typeof doneActuals[i] === 'number' ? t.targetSeconds - (doneActuals[i] as number) : 0);
            return (
              <li key={t.id} className="grid grid-cols-12 items-center p-3 gap-2">
                ...
                <div className={`col-span-2 text-right font-mono text-sm ${isActive ? 'text-black' : 'text-gray-500'}`}>{renderActual(i)}</div>
                <div className={`col-span-2 text-right font-mono text-sm ${isActive ? (remainingNow < 0 ? 'text-red-600' : 'text-green-700') : 'text-gray-500'}`}>{renderRemaining(i)}</div>
                {isActive && (
                  <div className="col-span-12 flex justify-end gap-2 mt-2">
                    <button className="px-3 py-1 rounded-lg border" onClick={skipCurrent}>⏭️ Skip</button>
                    <button className="px-3 py-1 rounded-lg bg-black text-white" onClick={completeCurrent}>✅ Done</button>
                  </div>
                )}
              </li>
            );
        ...
        <div className="grid grid-cols-12 px-3 py-2 border-t bg-gray-50 text-sm font-mono">
          ...
          <div className={`col-span-2 text-right ${totalRemaining < 0 ? 'text-red-600' : 'text-green-700'}`}>{fmt(totalRemaining)}</div>
        </div>
      </div>

      <div className="flex justify-end items-center gap-2">
        <button className="px-4 py-2 rounded-xl bg-red-600 text-white" onClick={() => setConfirmFinish(true)}>🏁 Finish</button>
        <button className="px-4 py-2 rounded-xl border" onClick={() => setShowHistory(true)}>📜 History</button>
        <button className="px-4 py-2 rounded-xl border" onClick={onEdit}>✏️ Edit</button>
        <button className="px-4 py-2 rounded-xl border" onClick={onClose}>✖️ Close</button>
      </div>
      ...
      {confirmFinish && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-md">
            <div className="px-5 py-4 border-b"><h3 className="text-lg font-semibold">Finish routine?</h3></div>
            <div className="px-5 py-4 text-sm">Are you sure you want to finish? Your current stats will be saved to history.</div>
            <div className="px-5 py-4 border-t flex justify-end gap-2">
              <button className="px-4 py-2 rounded-xl border" onClick={() => setConfirmFinish(false)}>Cancel</button>
              <button className="px-4 py-2 rounded-xl bg-red-600 text-white" onClick={() => { setConfirmFinish(false); finish(); }}>Finish</button>
            </div>
          </div>
        </div>
      )}
      ...
      {finishStats && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-md">
            <div className="px-5 py-4 border-b"><h3 className="text-lg font-semibold">Session summary</h3></div>
            <div className="px-5 py-4 text-sm">
              <div className="grid grid-cols-2 gap-y-2">
                <div className="text-gray-500">Date</div><div>{new Date(finishStats.dateISO).toLocaleDateString()}</div>
                <div className="text-gray-500">Start</div><div>{new Date(finishStats.startISO).toLocaleTimeString()}</div>
                <div className="text-gray-500">Finish</div><div>{new Date(finishStats.endISO).toLocaleTimeString()}</div>
                <div className="text-gray-500">Target</div><div className="font-mono">{fmt(finishStats.targetSeconds)}</div>
                <div className="text-gray-500">Actual</div><div className="font-mono">{fmt(finishStats.actualSeconds)}</div>
                <div className="text-gray-500">Δ</div><div className={`font-mono ${finishStats.deltaSeconds <= 0 ? 'text-green-700' : 'text-red-600'}`}>{fmt(Math.abs(finishStats.deltaSeconds))}</div>
                <div className="text-gray-500">Tasks</div><div>{finishStats.tasksCompleted}/{finishStats.tasksTotal}</div>
              </div>
            </div>
            <div className="px-5 py-4 border-t flex justify-end">
              <button className="px-4 py-2 rounded-xl bg-black text-white" onClick={() => setFinishStats(null)}>OK</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/30 grid place-items-center p-4 z-40">
          <div className="bg-white w-full max-w-2xl rounded-2xl border shadow-lg">
            <div className="flex items-center justify-between px-4 py-3 border-b">
              ...
              <button className="px-3 py-1 rounded border" onClick={() => setShowHistory(false)}>Close</button>
            </div>
                ...
                <tbody>
                  {routineSessions.map((s) => (
                    <tr key={s.id} className="border-t">
                      ...
                      <td className={`px-3 py-2 text-right font-mono ${s.deltaSeconds > 0 ? 'text-red-600' : 'text-green-700'}`}>{fmt(Math.abs(s.deltaSeconds))}</td>
                      <td className="px-3 py-2 text-right">{s.tasksCompleted}/{s.tasksTotal}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
const RoutineEditor: React.FC<{ draft: Routine; onChange: (r: Routine) => void; onSave: () => void; onClose: () => void }> = ({ draft, onChange, onSave, onClose }) => {
  const dragFrom = useRef<number | null>(null);
  const updateTask = (id: string, patch: Partial<Task>) => {
    onChange({ ...draft, tasks: draft.tasks.map(t => t.id === id ? { ...t, ...patch } : t), updatedAt: new Date().toISOString() });
  };
  const add = () => onChange({ ...draft, tasks: [...draft.tasks, { id: uid(), title: "New task", targetSeconds: 60 }], updatedAt: new Date().toISOString() });
  const del = (id: string) => onChange({ ...draft, tasks: draft.tasks.filter(t => t.id !== id), updatedAt: new Date().toISOString() });

  const onDragStart = (i: number) => (e: React.DragEvent) => { dragFrom.current = i; e.dataTransfer.effectAllowed = "move"; };
  const onDragOver = (e: React.DragEvent) => { e.preventDefault(); };
  const onDrop = (to: number) => (e: React.DragEvent) => {
    e.preventDefault(); const from = dragFrom.current; dragFrom.current = null; if (from == null || from === to) return;
    const arr = [...draft.tasks]; const [moved] = arr.splice(from, 1); arr.splice(to, 0, moved);
  ...
  return (
    <div className="space-y-4">
      <div>
        <input className="border rounded px-3 py-2 text-lg font-semibold w-full" value={draft.name}
      ...
      {draft.name === 'Morning Routine' && draft.tasks.length < seed.tasks.length && (
        <div className="p-3 rounded-xl border bg-amber-50 text-sm flex items-center justify-between">
          ...
          <button
            ...
            onClick={() => {
              const fresh = { ...draft, tasks: seed.tasks.map(t => ({ id: uid(), title: t.title, targetSeconds: t.targetSeconds })) };
      ...
      <div className="grid gap-2">
        {draft.tasks.map((t, i) => {
          ...
          return (
            <div key={t.id} className="p-2 rounded-xl border bg-white flex items-center gap-3"
              draggable onDragStart={onDragStart(i)} onDragOver={onDragOver} onDrop={onDrop(i)}>
              <div className="w-6 text-center"><DragHandle /></div>
              <input className="flex-1 border rounded px-2 py-1" value={t.title} onChange={(e) => updateTask(t.id, { title: e.target.value })} />
              ...
              <button className="px-2 py-1 rounded border text-red-600" onClick={() => del(t.id)}>Delete</button>
            </div>
          );
        })}
        <button className="px-3 py-2 rounded-xl border" onClick={add}>+ Add task</button>
      </div>

      <div className="flex justify-end gap-2">
        <button className="px-4 py-2 rounded-xl bg-black text-white" onClick={onSave}>Save</button>
        <button className="px-4 py-2 rounded-xl border" onClick={onClose}>✖️ Close</button>
      </div>
    </div>
  );
...
export default function Rituals() {
  ...
  const [mode, setMode] = useState<"home" | "view" | "edit">("home");
  const [activeId, setActiveId] = useState<string | null>(null);
  const active = routines.find((r) => r.id === activeId) || null;
  const [draft, setDraft] = useState<Routine | null>(null);

  const openNew = () => {
    const r: Routine = { id: uid(), name: "New routine", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), tasks: [] };
    setDraft(r); setMode("edit");
  };
  const openView = (r: Routine) => { setActiveId(r.id); setMode("view"); };
  const openEdit = (r: Routine) => { setDraft(JSON.parse(JSON.stringify(r))); setMode("edit"); };

  const saveDraft = () => {
    if (!draft) return;
    setRoutines((rs) => { const exists = rs.some((x) => x.id === draft.id); return exists ? rs.map((x) => (x.id === draft.id ? draft : x)) : [draft, ...rs]; });
    setTouched();
    setMode("view"); setActiveId(draft.id); setDraft(null);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      {mode === "home" && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            ...
            <button onClick={openNew} className="p-4 rounded-2xl border-dashed border-2 border-gray-300 hover:border-gray-400 text-left flex flex-col items-start justify-center min-h-[140px]">
      ...
      {mode === "view" && active && (
        <RoutineView routine={active} onEdit={() => openEdit(active)} onClose={() => { setMode("home"); setActiveId(null); }} sessions={sessions} onRecord={addSession} />
      )}

      {mode === "edit" && draft && (
        <RoutineEditor draft={draft} onChange={setDraft} onSave={saveDraft} onClose={() => { setMode("home"); setDraft(null); setActiveId(null); }} />
      )}

      <footer className="pt-6 text-center text-xs text-gray-500">Local‑first · Keep-style cards · Drag to reorder (Edit mode)</footer>
    </div>
  );
}
